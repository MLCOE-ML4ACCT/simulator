...
def load_noise_parameters(filename="noises/combined_noise_parameters.json"):
    ...
    Args:
        filename (str, optional): Path to JSON file.
            Defaults to "noises/combined_noise_parameters.json".
...
def create_gaussian_mixture_distribution(
    weights, means, stds, target_mean=None, target_variance=None
):
    ...
    Returns:
        tfp.distributions.Distribution: GMM distribution object (after linear transformation).
    """
    ...
    if target_mean is not None and target_variance is not None:
        ...
        transformed_gmm = tfd.TransformedDistribution(
            distribution=gmm, bijector=linear_transform
        )
...
def create_distribution_for_variable(var_name, var_params):
    """Create distribution for a single variable.

    For GMM, apply linear transformation to match target statistics.
...
def generate_all_distributions(parameters_file="noises/combined_noise_parameters.json"):
    ...
    Args:
        parameters_file (str, optional): Path to parameter file.
            Defaults to "noises/combined_noise_parameters.json".
