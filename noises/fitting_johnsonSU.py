...
def step1_initialize(mean, std, skew, kurt):
    ...
    Args:
        mean: Tensor of shape [batch_size] containing mean values.
        std: Tensor of shape [batch_size] containing standard deviation values.
        skew: Tensor of shape [batch_size] containing skewness values.
        kurt: Tensor of shape [batch_size] containing kurtosis values.

    Returns:
        tuple: (beta1, beta2) corresponding to Skew² and Kurt.
    """
...
def step2_find_omega1(beta2):
    ...
    Returns:
        tf.Tensor: omega1 - lower bound.
    """
    # According to the formula
    # D = (3+β₂)(16β₂²+87β₂+171)/27
    D = (
        (3.0 + beta2) * (16.0 * tf.square(beta2) + 87.0 * beta2 + 171.0) / 27.0
    )  # shape: [batch_size]
    ...
    cbrt_term1 = tf.sign(term1) * tf.pow(
        tf.abs(term1), 1.0 / 3.0
    )  # shape: [batch_size]
    cbrt_term2 = tf.sign(term2) * tf.pow(
        tf.abs(term2), 1.0 / 3.0
    )  # shape: [batch_size]
...
def step4_check_solution(omega1, beta1):
    ...
    Returns:
        tf.Tensor: Boolean tensor, True means solution exists, shape [batch_size].
    """
    # Compute (ω₁-1)(ω₁+2)²
...
def step5_calculate_omega2(beta2):
    ...
    Returns:
        tf.Tensor: omega2 - upper bound, shape [batch_size].
    """
...
def create_objective_function(beta1, beta2):
    ...
    def objective_fn(omega):
        ...
        Returns:
            tf.Tensor: Result tensor of shape [batch_size].
        """
        # m(ω) = -2 + √(4 + 2[ω² - (β₂+3)/(ω²+2ω+3)])
        omega_sq = tf.square(omega)  # shape: [batch_size]
        denominator = omega_sq + 2.0 * omega + 3.0  # shape: [batch_size]
...
def step6_find_omega_star(omega1, omega2, beta1, beta2):
    ...
    Returns:
        tf.Tensor: omega_star - found root, shape [batch_size].
    """
...
def step7_recover_parameters(omega_star, mean, std, skew, kurt, beta2):
    ...
    Args:
        ...
        std: Standard deviation, shape [batch_size].
        skew: Skewness, shape [batch_size].
        kurt: Kurtosis, shape [batch_size].
        beta2: Tensor of shape [batch_size].

    Returns:
        tuple: (delta, lambda_param, gamma, xi) where all are shape [batch_size].
    """
    # Ensure data type is float32
...
def johnson_su_fit_steps1to7(mean, std, skew, kurt):
    ...
    Returns:
        dict: Dictionary containing all intermediate results and final parameters.
    """
    ...
    omega_star = step6_find_omega_star(
    ...
    delta, lambda_param, gamma, xi = step7_recover_parameters(
        omega_star, mean, std, skew, kurt, beta2
    )  # shape: [batch_size]
...
def verify_johnson_su_fit(gamma, delta, xi, lambda_param, n_samples=10000, seed=42):
    ...
    Args:
        ...
        n_samples (int, optional): Number of samples per distribution. Defaults to 10000.
        seed (int, optional): Random seed. Defaults to 42.

    Returns:
        dict: Contains original parameters, empirical statistics, and comparison results.
    """
    print(
        f"Start verifying JohnsonSU fitting results, {n_samples} samples per distribution"
    )
...
def calculate_empirical_moments(samples):
    ...
    Returns:
        tuple: (emp_mean, emp_var, emp_skew, emp_kurt) where:
    ...
    emp_var = tf.reduce_mean(
        tf.square(samples - emp_mean), axis=0
    )  # shape: [batch_size]
    ...
    for var_name, var_data in flow_info.items():
        ...
        if method == "Tobit":
            ...
            if all(
                ...
                print(
                    f"  Extracted {var_name}: mean={var_data['mean']:.6f}, var={var_data['variance']:.6e}, skew={var_data['skewness']:.6f}, kurt={var_data['kurtosis']:.6f}"
                )
            else:
                print(
                    f"  Warning: {var_name} missing required four moments information"
                )
            ...
            if all(
                ...
                print(
                    f"  Extracted {var_name}_pos: mean={var_data['mean_pos']:.6f}, var={var_data['variance_pos']:.6e}, skew={var_data['skewness_pos']:.6f}, kurt={var_data['kurtosis_pos']:.6f}"
                )
            else:
                print(
                    f"  Warning: {var_name} missing positive distribution's four moments information"
                )
            ...
            if all(
                ...
                print(
                    f"  Extracted {var_name}_neg: mean={var_data['mean_neg']:.6f}, var={var_data['variance_neg']:.6e}, skew={var_data['skewness_neg']:.6f}, kurt={var_data['kurtosis_neg']:.6f}"
                )
            else:
                print(
                    f"  Warning: {var_name} missing negative distribution's four moments information"
                )
