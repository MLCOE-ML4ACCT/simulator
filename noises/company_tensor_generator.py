...
def generate_company_tensors(batch_size=10000):
    ...
    CA_past = tf.random.uniform(
        [batch_size, 1], minval=0.2 * 1000000, maxval=0.6 * 1000000, dtype=tf.float32
    )
    ...
    )  # Accumulative Supplementary Depreciation
...
if __name__ == "__main__":
    ...
    print(
        f"Unique values in largcity: {tf.unique(tf.reshape(largcity, [-1])).y.numpy()}"
    )
    print(
        f"Unique values in ruralare: {tf.unique(tf.reshape(ruralare, [-1])).y.numpy()}"
    )
    print(f"Count of largcity=1: {int(tf.reduce_sum(largcity).numpy())}")
    print(f"Count of ruralare=1: {int(tf.reduce_sum(ruralare).numpy())}")
    print(
        f"Count of both being 0: {int(tf.reduce_sum((1-largcity)*(1-ruralare)).numpy())}"
    )
