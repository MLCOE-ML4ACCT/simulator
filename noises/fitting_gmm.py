...
class TrainingConfig:
    ...
    def __init__(self, target_moments=None):
        ...
        Args:
            target_moments (dict, optional): Target moments dictionary containing mean,
                variance, skewness, kurtosis. Defaults to None.
        """
...
class MomentMatchingGMM:
    ...
    def __init__(self, num_components, config):
        ...
        self.locs = tf.Variable(
            np.random.normal(0, 3, num_components).astype(np.float32), name="locs"
        )
        ...
        self.scales = tf.Variable(
            np.ones(num_components, dtype=np.float32) * initial_scale, name="scales"
        )

        # 3. Weights: parameterized by logits, converted to weights by softmax
        self.logits = tf.Variable(
            np.zeros(num_components, dtype=np.float32), name="logits"
        )
    ...
    def get_gmm_distribution(self):
        ...
        Uses logits+softmax for weights and abs for stddevs to ensure positivity.

        Returns:
            tfd.MixtureSameFamily: The Gaussian mixture distribution.
        """
...
def get_closed_form_moments(gmm_dist):
    ...
    gmm_variance = tf.reduce_sum(
        weights * (component_variances + tf.square(component_means))
    ) - tf.square(gmm_mean)
    gmm_stddev = tf.sqrt(
        gmm_variance + 1e-10
...
def compute_empirical_moments(distribution, num_samples=200000):
    ...
    Args:
        ...
        num_samples (int, optional): Number of samples. Defaults to 200000.
...
def create_tensor_dict(tensor_dict, convert_to_float=False):
    ...
    Args:
        ...
        convert_to_float (bool, optional): Whether to convert tensors to float.
            Defaults to False.
...
def compute_loss(model_moments, target_moments):
    ...
    Args:
        model_moments (dict): Dictionary containing computed model moments.
    ...
    skewness_diff = (
        model_moments["skewness"] - target_moments["skewness"]
    ) / target_moments["skewness"]
    skewness_loss = tf.square(skewness_diff)

    # Kurtosis loss: relative error on kurtosis
    kurtosis_diff = (
        model_moments["kurtosis"] - target_moments["kurtosis"]
    ) / target_moments["kurtosis"]
...
def train_step_core(model, target_moments):
    ...
    Returns:
        tuple: (loss, model_moments, gradients).
    """
...
def create_optimizer_and_scheduler(config):
    ...
    lr_schedule = tf.keras.optimizers.schedules.CosineDecayRestarts(
        ...
        / config.MAX_LEARNING_RATE,  # Minimum learning rate ratio
    )
...
def train_model(model, optimizer, target_moments, config):
    ...
    print(
        f"Target moments to fit: Skew={target_moments['skewness']:.4f}, "
    ...
    for step in range(config.TRAINING_STEPS):
        loss_val, moments_val = train_step(model, optimizer, target_moments)
        ...
        if step % 200 == 0 or step == config.TRAINING_STEPS - 1:
            ...
            print(
                f"         | Mean: {moments_val['mean'].numpy():.4f}, Var: {moments_val['variance'].numpy():.4e}"
            )
            print(
                f"         | Skew: {moments_val['skewness'].numpy():.4f} (Target: {target_moments['skewness']:.4f})"
            )
            print(
                f"         | Kurt: {moments_val['kurtosis'].numpy():.4f} (Target: {target_moments['kurtosis']:.4f})"
            )
            print("-" * 100)
...
def postprocess_model(model, target_moments):
    ...
    Returns:
        tuple: (final_gmm, sorted_weights, sorted_means, sorted_stddevs, a, b)
    ...
    target_std_tensor = tf.sqrt(
        tf.constant(target_moments["variance"], dtype=tf.float32)
    )
    ...
    final_gmm = tfd.TransformedDistribution(
        distribution=fitted_gmm, bijector=linear_transform
    )
    ...
    print(
        "(Note: These parameters are for display only. Actual distribution uses TransformedDistribution)"
    )
    for i in range(model.num_components):
        print(
            f"  Component {i+1}: Weight={sorted_weights[i]:.4f}, "
...
def print_training_tips():
    ...
    print(
        "3. Focus on skewness and kurtosis fitting - mean and variance will be automatically corrected"
    )
...
def fit_single_variable(var_name, var_data, suffix=""):
    ...
    Args:
        ...
        suffix (str, optional): Suffix (for pos/neg distinction). Defaults to "".
    ...
    fitted_skewness, fitted_kurtosis = get_closed_form_moments(
        fitted_gmm_before_transform
    )
    ...
    fitted_stats = create_tensor_dict(
        {"skewness": fitted_skewness, "kurtosis": fitted_kurtosis},
        convert_to_float=True,
    )

    print(f"Final distribution statistics (empirical, after linear transformation):")
    print(
        f"  Mean: {empirical_stats_dict['mean']:.4f} (Target: {target_moments['mean']:.4f})"
    )
    print(
        f"  Var:  {empirical_stats_dict['variance']:.4e} (Target: {target_moments['variance']:.4e})"
    )
    print(
        f"  Skew: {empirical_stats_dict['skewness']:.4f} (Target: {target_moments['skewness']:.4f})"
    )
    print(
        f"  Kurt: {empirical_stats_dict['kurtosis']:.4f} (Target: {target_moments['kurtosis']:.4f})"
    )
    print(f"Fitted skewness and kurtosis (before linear transformation):")
    print(
        f"  Fitted Skew: {fitted_stats['skewness']:.4f} (Target: {target_moments['skewness']:.4f})"
    )
    print(
        f"  Fitted Kurt: {fitted_stats['kurtosis']:.4f} (Target: {target_moments['kurtosis']:.4f})"
    )
...
def process_all_variables():
    ...
    for var_name, var_data in Flow_info.items():
        ...
        if method == "Tobit":
            ...
            results[var_name]["scale"] = (
                float(scale_value) if scale_value is not None else None
            )
            continue
            ...
            if all(
                key in var_data for key in ["mean", "variance", "skewness", "kurtosis"]
            ):
                fit_result = fit_single_variable(var_name, var_data)
                results[var_name].update(fit_result)
            else:
                print(
                    f"Warning: Variable {var_name} missing necessary moment information"
                )
            ...
            if all(
                key in var_data
                for key in ["mean_pos", "variance_pos", "skewness_pos", "kurtosis_pos"]
            ):
                ...
                results[var_name]["empirical_stats_pos"] = fit_result_pos[
                    "empirical_stats"
                ]
            ...
            if all(
                key in var_data
                for key in ["mean_neg", "variance_neg", "skewness_neg", "kurtosis_neg"]
            ):
                ...
                results[var_name]["empirical_stats_neg"] = fit_result_neg[
                    "empirical_stats"
                ]
...
def save_results_to_json(
    results,
    parameters_filename="gmm_parameters.json",
    statistics_filename="gmm_statistics.json",
):
    """Save results to two separate JSON files: one for parameters and one for statistics.

    Args:
        results (dict): Fitting results dictionary.
        parameters_filename (str, optional): Output filename for parameters.
            Defaults to "gmm_parameters.json".
        statistics_filename (str, optional): Output filename for statistics.
            Defaults to "gmm_statistics.json".
    """
