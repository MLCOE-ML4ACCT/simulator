...
def huber_schweppe_estimator(
    X_train_np, y_train_np, max_iterations=100, tol=1e-6, k=1.345
):
    """
    ...
    for i in range(max_iterations):
        ...
        print(f"Iter {i+1:2d}: Change in coefficients = {diff:.8f}")
        if diff < tol:
...
if __name__ == "__main__":
    # --- Example Usage ---
    # Create some sample data with an outlier
    np.random.seed(42)
    X_sample = np.random.rand(100, 2) * 10
    # True relationship: y = 2 + 3*x1 + 5*x2 + noise
    y_sample = 2 + 3 * X_sample[:, 0] + 5 * X_sample[:, 1] + np.random.randn(100) * 2
    ...
    # Estimate coefficients using the robust estimator
    learned_weights = huber_schweppe_estimator(X_sample, y_sample)

    print("\nLearned Coefficients (Beta) from Huber-Schweppes Estimator:")
    print(f"Intercept: {learned_weights[0].numpy()[0]:.4f}")
    print(f"Weight for Feature 1: {learned_weights[1].numpy()[0]:.4f}")
    print(f"Weight for Feature 2: {learned_weights[2].numpy()[0]:.4f}")

    # For comparison, let's do a standard OLS
    X_sample_b = np.hstack([np.ones((100, 1)), X_sample])
    ols_weights = np.linalg.inv(X_sample_b.T @ X_sample_b) @ (X_sample_b.T @ y_sample)
    print("\nFor Comparison: OLS Coefficients (influenced by outlier):")
    print(f"Intercept: {ols_weights[0][0]:.4f}")
    print(f"Weight for Feature 1: {ols_weights[1][0]:.4f}")
    print(f"Weight for Feature 2: {ols_weights[2][0]:.4f}")
