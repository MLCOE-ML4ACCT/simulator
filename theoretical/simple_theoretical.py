...
class SimulatorEngine(tf.keras.models.Model):
    ...
    def call(self, input_t_1: dict, training=False) -> dict:
        ...
        # Debug: Print out the balance difference for firms where the balance is not acceptable
        if not tf.reduce_all(is_balance_acceptable):
            unacceptable_indices = tf.where(tf.logical_not(is_balance_acceptable))
            tf.print("unacceptable_indices:", unacceptable_indices.shape)
